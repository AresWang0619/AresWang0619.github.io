...
> ## KMP算法 <a name="KMP算法"></a>
>
> ![KMP算法](image-2.png)
>
> KMP算法是一种字符串匹配算法，时间复杂度在O(m+n)。
>
> 1. `s[]` 是主串，即比较长的字符串。
> 2. `p[]` 是模式串，即比较短的字符串。
...
> 6. `next[]` 是“部分匹配值表”，即 `next` 数组，它存储的是每一个下标对应的“部分匹配值”，是KMP算法的核心。用于存储前后缀的相等长度`j`。
>
> 核心思想：在每次失配时，只前进，不后移，前进的距离取决于 `next[j]`。
>
> ```cpp
> #include<iostream>
> using namespace std;
...
>
>     //求next[]数组
>     for (int i = 2, j = 0; i <= n; i++) { //因为ne[0]=0,ne[1]回退看ne[0]=0,所以直接从2开始。
>         while (p[i] != p[j + 1] && j) 
>             j = ne[j]; //匹配不成功反复使得j回退直到匹配成功
>         if (p[i] == p[j + 1]) 
>             j++;
>         ne[i] = j;
>     }
...
>         while (j && s[i] != p[j + 1]) //如果j存在指向的字符，且指向的字符不匹配，进行ne[j]查询前进格子数。
>             j = ne[j];
>         if (s[i] == p[j + 1]) //匹配则移动到下一位
>             j++;
>         if (j == n) { //匹配成功
>             printf("%d ", i - n); //输出以0开始的匹配子串的首字母下标，所以不需要+1
>             j = ne[j]; //继续匹配下一个子串
>         }
>     }
>     return 0;
...
>        int x;
>        cin >>x;
>        while(tt&&stk[tt]>=x) tt--; //当指针存在但是元素大小不满足的时候直接剔除
>        if(tt) cout<<stk[tt]<<' '; //存在则输出
>        else cout<<-1<<' '; //不存在输出-1
>        stk[++tt]=x; //满足的x入栈
>    }
>    return 0;
...
>### 单调队列 <a name="单调队列"></a>
>
>给出一个序列，有n个元素，滑动窗口大小为k，求滑动窗口中的max和min。
>
> ```cpp
> #include<iostream>
> using namespace std;
...
>    int hh=0,tt=-1; //队头和队尾
>    for(int i=0;i<n;i++){
>        if(hh<=tt&&i-k+1>q[hh]) hh++; //在队列不为空且i不超出左边界的情况下，往前移动
>        while(hh<=tt&&a[q[tt]]>=a[i]) tt--; //删除不符合的元素
>        q[++tt]=i; //将合适的元素加入队列
>        if(i>=k-1) printf("%d ",a[q[hh]]); //达到滑动窗口大小后输出最小的值
>    }
>    return 0;
> }
> ```
